...
class StandardScaler:
    ...
    def inverse_transform(self, data, which_dim = None): # which_dim 
        if which_dim ==None:
            print(None)
            temp_std = self.std
            temp_mean = self.mean
        else:
            temp_std = self.std[...,which_dim:which_dim+1]                                # (1, num_vertices, num_features) -> (1, num_vertices, 1)
            temp_mean = self.mean[...,which_dim:which_dim+1]                              # (1, num_vertices, num_features) -> (1, num_vertices, 1)
        if type(data) == torch.Tensor and type(self.mean) == np.ndarray: 
            temp_std = torch.from_numpy(temp_std).to(data.device).type(data.dtype) 
            temp_mean = torch.from_numpy(temp_mean).to(data.device).type(data.dtype)
        elif type(data) == torch.Tensor and type(self.mean) == torch.Tensor: 
            temp_std = temp_std.to(data.device).type(data.dtype) 
            temp_mean = temp_mean.to(data.device).type(data.dtype)
        return (data * temp_std) + temp_mean
...
class MinMax01Scaler:
    ...
    def inverse_transform(self, data, which_dim = None): # which_dim
        if which_dim ==None:
            ...
            temp_min = self.min[...,which_dim:which_dim+1]                             # (1, num_vertices, num_features) -> (1, num_vertices, 1)
            temp_max = self.max[...,which_dim:which_dim+1]                             # (1, num_vertices, num_features) -> (1, num_vertices, 1)
        if type(data) == torch.Tensor and type(self.min) == np.ndarray:   
            temp_min = torch.from_numpy(temp_min).to(data.device).type(data.dtype) 
            temp_max = torch.from_numpy(temp_max).to(data.device).type(data.dtype)
        elif type(data) == torch.Tensor and type(self.mean) == torch.Tensor:
            temp_min = temp_min.to(data.device).type(data.dtype)
            temp_max = temp_max.to(data.device).type(data.dtype)
        return (data * (temp_max - temp_min) + temp_min)
...
class MinMax11Scaler:
    ...
    def inverse_transform(self, data, which_dim = None):
        if which_dim ==None:
            temp_min = self.min
            temp_max = self.max
        else:
        ...
        if type(data) == torch.Tensor and type(self.min) == np.ndarray:
            ...
            temp_min = temp_min.to(data.device).type(data.dtype)
            temp_max = temp_max.to(data.device).type(data.dtype)
        return ((data + 1.) / 2.) * (temp_max - temp_min) + temp_min
    
def normalize_dataset(data, normalizer, column_wise=True):  
    if normalizer == 'max01':
        if column_wise:
        ...
        if column_wise:
            mean = data.mean(axis=0, keepdims=True) # data: (num_timesteps, num_vertices, num_features) -> data.mean: (1, num_vertices, num_features)
            std = data.std(axis=0, keepdims=True)   # data: (num_timesteps, num_vertices, num_features) -> data.std: (1, num_vertices, num_features)
        else:
            mean = data.mean()
            std = data.std()
...
def generate_dataset(X, args, normalizer):
    ...
    # normalize_dataset 
    X, scaler = normalize_dataset(X, normalizer)
    
    num_timesteps_input = args.T
    num_timesteps_output = args.l
    # Generate the beginning index and the ending index of a sample, which
    # contains (num_points_for_training + num_points_for_predicting) points
    intervals = [(i, i + (num_timesteps_input + num_timesteps_output)) for i in range(X.shape[0] - (num_timesteps_input + num_timesteps_output) + 1)] # X.shape[0] 表示 num_timesteps 时间步总数
    ...
    return torch.stack(features), torch.stack(target), scaler
...
def generate_dataset_for_each_datatype(args):
    ...
    ds_feature, ds_target, scaler =  generate_dataset(ds_matrix_t, args,'std') 
    ...
    outbreak_feature,_,_ = generate_dataset(outbreak_matrix_t, args, 'std')
    ...
    risk_feature,_,_ = generate_dataset(risk_matrix_t, args, 'std') 
    ...
    abnormal_news_num_matrix = abnormal_news_num_matrix.repeat(1, 282, 1)
    abnormal_news_num_feature,_,_= generate_dataset(abnormal_news_num_matrix, args, 'std') 
    ...
    festival_feature,festival_target,_= generate_dataset(festival_matrix, args, 'None')
    ...
    mask_value = torch.cat([torch.tensor([101,102,]), torch.full((68,),0)]).repeat(608,24,1)
    ...
    daily_news_input_ids_feature,_ ,_ =  generate_dataset(daily_news_input_ids, args,'None') 
    daily_news_token_type_ids_feature,_ ,_ =  generate_dataset(daily_news_token_type_ids, args,'None') 
    daily_news_attention_mask_feature,_ ,_ =  generate_dataset(daily_news_attention_mask, args,'None') 
    daily_valid_news_mask_feature,_,_ = generate_dataset(daily_valid_news_mask, args,'None') 
     
    return ds_feature, ds_target, scaler, outbreak_feature, risk_feature, news_matrix_fine_tuning, daily_news_input_ids_feature, daily_news_token_type_ids_feature, daily_news_attention_mask_feature, daily_valid_news_mask_feature, abnormal_news_num_feature, festival_feature, festival_target
...
def save_metrics(args, model_name, d_or_s, MAE, RMSE, sMAPE): 
    with open(args.log_dir + '/metrics.csv', 'a', encoding="gbk", newline="") as f:
        csv_writer = csv.writer(f)
        # csv_writer.writerow(["Model", "d_or_s", "MAE", "RMSE", "sMAPE", ])
        csv_writer.writerow([model_name, d_or_s, MAE, RMSE, sMAPE])
